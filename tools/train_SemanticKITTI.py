...
class Trainer:
    def __init__(self):
        ...
        config_module = importlib.import_module(f'config.{FLAGS.model_config}')
        ...
        # Network & Optimizer
        self.net = Network(self.model_cfg)
        ...
        self.optimizer = optim.Adam(self.net.parameters(), lr=self.model_cfg.learning_rate)
    ...
    def validate(self):
        ...
        iou_calc = IoUCalculator(self.model_cfg, FLAGS.distributed, FLAGS.device)
        ...
        with torch.no_grad():
            for batch_idx, batch_data in enumerate(tqdm_loader):
                ...
                acc, end_points = compute_acc(end_points) # ACC = (TP+TN)/(TP+TN+FP+FN)
                val_acc_total += acc
                iou_calc.add_data(end_points)

        mean_loss = val_loss_total / len(self.val_loader)
        mean_acc = val_acc_total / len(self.val_loader) 
        mean_iou, iou_list = iou_calc.compute_iou() # IoU=TP/(TP+FP+FN)

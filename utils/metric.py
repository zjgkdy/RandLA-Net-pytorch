        ...
        for n in range(0, self.cfg.num_classes, 1):
            if float(self.gt_classes[n] + self.positive_classes[n] - self.true_positive_classes[n]) != 0:
                iou = self.true_positive_classes[n] / \
                    float(self.gt_classes[n] + self.positive_classes[n] - self.true_positive_classes[n]) # IoU=TP/(TP+FP+FN)
                iou_list.append(iou)
            else:
                iou_list.append(0.0)
...
def compute_acc(end_points):
    ...
    acc = (logits == labels).sum().float() / float(labels.shape[0]) # ACC = (TP+TN) / (TP+TN+FP+FN)
    end_points['acc'] = acc
